...
def fetch_structure(
    ...
    if source_preference == "rdkit" and not png_bytes and cid:
        # rdkit preference but failed — try PubChem as fallback
        png_bytes = _fetch_pubchem_png(cid, size=size)
        if png_bytes:
...
def fetch_all_structures(
    ...
    for i, cache_file in enumerate(cache_files, 1):
        ...
        if path:
            ...
            print(f"  [{i:4d}/{total}] ✓  {drug_name[:45]:45s} [{src}]")
        else:
            summary["failed"] += 1
            print(f"  [{i:4d}/{total}] ✗  {drug_name[:50]}")
...
if __name__ == "__main__":
    ...
    if path:
        ...
        print("✗ Failed to fetch structure image")